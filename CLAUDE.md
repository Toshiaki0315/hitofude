...
### R9. `.OboeGaki` の索引ファイルは捨ててよいキャッシュ（§7.1）
...
索引の名前は `storage/index_db.INDEX_FILE` が**唯一の出所**。書き写さない。
素の `index.sqlite` は**作り直し版（Tauri/Rust）のもの**なので使わない
（ADR-0034）。同じ保管フォルダを共有していると、名前を分けない限り
双方が「知らない形」として作り直し合う。
...
実測は `make bench`（3 つとも測って基準と突き合わせる）。
