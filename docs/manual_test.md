...
| [ ] | 本文で `[[` と打つ                                          | 既にあるノートの題名が候補に出る。選ぶと `]]` まで入る                                                     |
| [ ] | `Cmd+Shift+F` で `#タグ 言葉` と打つ                        | そのタグのノートの中だけが出る                                                                             |
| [ ] | `Cmd+Shift+F` でタグだけ打つ                                | そのタグのノートが並ぶ（親を打つと子も出る）                                                               |
| [ ] | `after:2026-08-01` と打つ                                   | その日以降に更新したノートだけが出る（その日も含む）                                                       |
| [ ] | `after:きのう` と打つ                                       | 検索欄の下に書き方の案内が出る。絞り込みはされず、その文字列を探す                                         |
| [ ] | 正しい日付に打ち直す                                        | 案内が消える                                                                                               |
| [ ] | 書いて保存し、5 分以上あけて書き直す                        | 「版の履歴…」に 2 つ並ぶ                                                                                  |
| [ ] | 続けて何度も保存する                                        | 版は増えない（5 分は 1 つ）                                                                                |
| [ ] | 古い版を選んで「この版に戻す」                              | 本文が戻り、戻す前の内容も版に残る                                                                         |
| [ ] | 版の中身を見る                                              | front matter（id など）は出ない                                                                            |
| [ ] | `Cmd+5` を押す                                              | 本文の右に見出しの一覧が出る（階層は字下げ）                                                               |
| [ ] | Raw の隣のアウトラインのボタンを押す                        | 開閉する。開いている間は押された形になる                                                                   |
| [ ] | 歯車 →「アウトライン」                                     | 開閉する。印が今の状態と合っている                                                                         |
| [ ] | `Cmd+5` で開いてボタンを見る                                | どこから開いてもボタンの形が揃う                                                                           |
| [ ] | 見出しを押す                                                | その場所へ飛ぶ                                                                                             |
| [ ] | 見出しを書き足す                                            | 一覧が追いつく                                                                                             |
| [ ] | 出したまま再起動                                            | 開いた状態で出る（`Cmd+5` で閉じられる）                                                                   |
| [ ] | 日次ノートを 2 日ぶん作って `Cmd+Shift+[`                   | 前に書いた日のノートが開く（間の日は飛ばす）                                                               |
| [ ] | いちばん古い日で `Cmd+Shift+[`                              | 「これより前の日のノートはありません」と出て、ノートは増えない                                             |
| [ ] | 右クリック →「複製」                                       | 写しが開く。見出しも新しい名前になっている                                                                 |
| [ ] | Finder でフォルダを作りノートを入れる                       | 一覧・検索に出る。開ける                                                                                   |
| [ ] | そのノートの名前を変える / 複製する                         | **フォルダに留まる**（直下へ出ない）                                                                       |
| [ ] | 右クリック →「リンクをコピー」                             | 別のノートに貼ると `[[名前]]` になる                                                                       |
| [ ] | 右クリック →「Finder で表示」                              | その `.md` が選ばれた状態で Finder が開く                                                                  |
...
| [ ] | 同じサブフォルダを作った直後にサイドバーを見る         | 「フォルダ」の見出しの下に出て、件数が付く（K-2）                                                                        |
| [ ] | サイドバーでそのフォルダを選ぶ                         | その中のノートだけが一覧に並ぶ（子フォルダのぶんも入る）                                                                 |
| [ ] | 「すべて」に戻して一覧を見る                           | サブフォルダのノートの行に、日付の左へ `仕事` と小さく出る                                                               |
