...
| [ ] | `[[ノート名]]` を書いて行から離れる                    | `[[` `]]` が消え、名前がリンク色になる                                                                     |
| [ ] | 指されている側のノートを開く                           | 本文の下に「▸ バックリンク N」の帯が出る（0 件なら出ない）                                                 |
| [ ] | `Cmd+P` を押す                                         | **印刷ダイアログ**が出る（PDF 書き出しではない）                                                           |
| [ ] | 印刷ダイアログの「PDF」→「PDF として保存」            | 書き出した PDF と同じものが出る（A4・余白 18mm）                                                           |
...
| [ ] | Finder でノートをリネームする（見出しあり）            | **一覧の題名は変わらない**（題名は本文の H1。§7.2）。旧名の行が残らない（索引側は自動: tests/storage/test_index_db.py） |
| [ ] | 開いているノートを Finder でリネームする               | 旧パスの削除として扱われ「作り直しますか？」と聞かれる                                                                   |
...
| [ ] | iCloud Drive 配下に保管フォルダを置いて使う            | 保存・再読み込みが破綻しない                                                                                             |
