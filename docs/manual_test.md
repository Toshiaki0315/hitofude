...
| [ ] | 環境設定で「行間」を ゆったり にする                        | サイドバーと一覧の行が広がる（本文は変わらない）                                                           |
| [ ] | 環境設定を開く                                              | 行が詰まっておらず、保管フォルダのパスが潰れていない                                                       |
| [ ] | 保管フォルダに `~/Documents/別の場所` と打つ                | OK で受け取られ、再起動の案内が出る                                                                        |
| [ ] | 保管フォルダに `メモ` と打って OK                           | 使えないと知らせが出て、ダイアログは閉じない                                                               |
| [ ] | `.app` から起動して「変更…」を押す                         | ファイルを選ぶ画面が**日本語**（開く / キャンセル / 新規フォルダ）。`make run` では英語のまま              |
| [ ] | 詰めて にして再起動                                         | 選んだ行間のまま開く                                                                                       |
...
| [ ] | 空のゴミ箱 / お気に入り / タグを開く                        | 案内の文言がそれぞれ変わる（ゴミ箱で「＋ で作れます」と出ない）                                            |
...
| [ ] | ゴミ箱が空の状態で同じ右クリック                            | 「ゴミ箱を空にする…」が灰色で押せない                                                                     |
...
| [ ] | どれか 1 つの形式で書き出す                                 | 画面の下に保存先（`~/…`）が出て、右に「Finder で表示」が出る                                              |
| [ ] | その「Finder で表示」を押す                                 | Finder が開き、**書き出したファイルが選ばれている**                                                        |
| [ ] | しばらく待つ                                                | 知らせと「Finder で表示」が消える（5 秒）                                                                  |
| [ ] | `Cmd+＋` を数回押す                                         | 本文が大きくなり、見出しやコードも一緒に大きくなる                                                         |
| [ ] | `Cmd+0` を押す                                              | 標準（15pt）に戻る                                                                                         |
| [ ] | 大きくしてからアプリを再起動                                | その大きさで開く。環境設定の「文字サイズ」も同じ値                                                         |
