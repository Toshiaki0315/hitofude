...
| [ ] | 本文を右クリックする                                       | 「元に戻す」「切り取り」などが日本語で出る                                                                 |
| [ ] | 一覧・フォルダを右クリックする                             | 本文の右クリックと同じ大きさ・同じ間隔で出る                                                               |
| [ ] | その項目の左を見る                                         | アイコンが付いている（本文のメニューと同じ形）                                                             |
| [ ] | その項目の文字をよく見る                                   | `(&U)` のような飾りが付いていない                                                                          |
| [ ] | 「はい / いいえ」を聞くダイアログを出す                    | ボタンも日本語（例: ゴミ箱を空にする）                                                                     |
...
| [ ] | 本文の一部を選んで `Cmd+Shift+K`（仮身化 / M-1）            | 選んだ文が消えて `[[題名]]` が残る。**切り出した先は開かない**（書いている流れが切れない）                  |
| [ ] | 続けて `Cmd+Z` を 1 回                                      | **1 回で元に戻る**（半端に戻らない）（自動: tests/ui/test_extract_selection.py）                            |
| [ ] | 残った `[[題名]]` を Cmd+クリック                           | 切り出したノートが開く（**新しく作られない**）                                                             |
| [ ] | 日本語を変換中（未確定）に `Cmd+Shift+K`                    | **何も起きない**（R6。プリエディットが壊れない）                                                           |
| [ ] | 何も選ばずに「編集」メニューを開く                          | 「選択範囲をノートにする」が灰色                                                                           |
| [ ] | `Cmd+Shift+R`（リンクの図 / M-2）                           | 別窓が開き、**今のノートが赤く大きい点**で中央にある                                                       |
| [ ] | 図の「深さ」を 1 と 3 に変える                              | 点が減る / 増える。閉じて開き直しても**選んだ深さが残る**                                                  |
| [ ] | 図の点を押す                                                | そのノートが開き、図は閉じる                                                                               |
| [ ] | **中抜きの点**（まだ無いノート）を押す                      | **作られない**。「まだ無いノートです」と出る                                                               |
| [ ] | `- 参考文献: [[どれか]]` と書いて図を開く（続柄 / M-3）     | 「すべての続柄」の隣に**続柄の選択肢**が出る                                                                |
| [ ] | 続柄を選ぶ                                                  | その関係のリンクだけの図になる。**起点は残る**                                                             |
| [ ] | `- 10:30 の打ち合わせ [[どれか]]` と書く                    | 「10」が続柄にならない（選択肢に出ない）                                                                   |
| [ ] | 続柄を使っていない vault で図を開く                         | 続柄の選択肢そのものが出ない                                                                               |
| [ ] | 長い題名のノートを何本か指してから図を開く（M-4）           | 題名が**窓からはみ出さない**。長いものは真ん中が `…` になる                                                 |
| [ ] | その `…` の付いた題名を読む                                 | **末尾が残っている**（連番や日付で見分けられる）                                                           |
| [ ] | 図の窓を小さくしてから開く                                  | 題名がはみ出さない                                                                                         |
| [ ] | 100 本以上と繋がったノートで図を開く                        | **題名同士が重ならない**（点は出るが題名の無いものがある）                                                  |
| [ ] | 題名の出ていない点にカーソルを乗せる                        | その点の題名だけが出る                                                                                     |
| [ ] | 窓を大きくする                                              | 出る題名が増える                                                                                           |
| [ ] | 版を上げた後に初めて起動する（M-5）                         | **ノートが一覧に出る**（索引は作り直され、少し待つ）                                                       |
| [ ] | `.hitofude/index.sqlite` を消してから起動                   | 一覧が戻る（捨ててよいキャッシュ。R9）                                                                     |
| [ ] | アプリを閉じている間に Finder で `.md` を足し、開いてから「ファイル」→「最新の情報に同期」（M-6） | 足したノートが一覧に出る。「1 件増えました」と出る                                                          |
| [ ] | 同じく Finder で 1 つ消してから「最新の情報に同期」        | 一覧から消える。「1 件消えました」と出る                                                                   |
| [ ] | 何も変えずに「最新の情報に同期」                            | **「変わりはありません」**と出る（無反応に見えない）                                                       |
| [ ] | 打ちかけの字がある状態で「最新の情報に同期」                | **打ちかけが消えない**（先に保存される）                                                                   |
| [ ] | 「ファイル」→「索引を作り直す」                            | 一覧が保たれる。「N 件増えました」と出る。**ファイルは減らない**                                            |
| [ ] | 文字を選んで `Cmd+Shift+X`                                  | **打ち消し線**（切り出しではない。M-1 で取り違えていたキー）                                                |
| [ ] | 「ファイル」→「版の履歴…」                                 | 開く。**キーは表示されない**（`Cmd+Shift+H` はハイライトなので取り下げた）                                  |
...
| [ ] | `Cmd+Shift+F` の右上を見る                                  | 「閉じる」ボタンがあり、押すと閉じる                                                                                   |
| [ ] | `×` を押さずに `Esc`                                        | 今まで通り閉じる                                                                                           |
...
| [ ] | `samples/会議メモ.png` を読み込む                          | 文字が読み取られてノートになる（ADR-0027）                                                                 |
| [ ] | `samples/会議メモ-スキャン.pdf` を読み込む                 | 絵だけの PDF もページごとに読み取られる                                                                    |
| [ ] | `samples/会議メモ.pdf` を読み込む                          | 速い（読み取りに回らない）                                                                                 |
| [ ] | フォルダを選んだ状態で読み込む                             | そのフォルダの中にノートができる                                                                           |
| [ ] | フォルダを選んだ状態で `.md` をドロップ                    | 同じくそのフォルダに入る                                                                                   |
| [ ] | `samples/会議メモ-図つき.pdf` を読み込む                   | 図が `attachments/` に入り、本文の後ろに出る                                                               |
| [ ] | スキャンした PDF を読み込む                                | 紙の写真は添付されない（文字だけ）                                                                         |
| [ ] | `samples/会議メモ-混在.pdf` を読み込む                     | どちらのページも中身が入る                                                                                 |
| [ ] | 設定で「文字の読み取り」を ローカルLLM にする              | 画像を読めるモデルなら読める。遅い                                                                         |
| [ ] | 読み取りができない状態で画像を読み込む                     | ノートを作らず、確かめる場所を知らせる                                                                     |
...
| [ ] | フォルダを右クリックする                               | 「名前を変更…」が「フォルダを削除…」の上にある                                                                           |
| [ ] | 名前を変えて OK                                        | サイドバーも一覧も新しい名前になる                                                                                       |
| [ ] | そのフォルダのノートを開いたまま名前を変える           | 開いたまま編集でき、保存も通る                                                                                           |
| [ ] | 既にある名前に変えようとする                           | 断られ、フォルダは元のまま                                                                                               |
