...
| [ ] | 一覧のノートをフォルダへドラッグする                   | 落とす先のフォルダの行が塗られる（どこへ入るか分かる）                                                                   |
| [ ] | 運んでいる間、今いるフォルダを見る                     | 帯が消えていて、光っているのは行き先だけ                                                                                 |
| [ ] | フォルダの外で離す（移動をやめる）                     | 帯が今いるフォルダに戻る。一覧は変わらない                                                                               |
| [ ] | フォルダへ落とす                                       | 行き先のフォルダが開き、運んだノートが選ばれている                                                                       |
| [ ] | 右クリック →「フォルダへ移動…」で移す                  | 落としたときと同じく行き先のフォルダが開く                                                                               |
| [ ] | `仕事/` のノートに `[[まだ無い]]` と書いて Cmd+クリック | `仕事/` の中にできる（一覧をどこで絞っていても同じ）                                                                     |
| [x] | フォルダを右クリック →「Finder で開く」                | そのフォルダが Finder で開く（直下なら保管フォルダ）                                                                     |
| [ ] | サイドバーのフォルダとタグを見る                       | 件数が右端に薄く出る（名前と離れている）                                                                                 |
| [ ] | `テスト２` のように数字で終わる名前を作る              | 名前と件数が読み分けられる                                                                                               |
| [ ] | サイドバーを狭める                                     | 名前が `…` で切れ、件数は消えない                                                                                        |
| [ ] | フォルダで絞った状態で `[[別フォルダのノート]]` を Cmd+クリック | そのノートのフォルダへ左の選択も一覧も移る                                                                               |
| [ ] | 同じフォルダの中のノートへ Cmd+クリック                | 絞り込みは変わらない（帯だけ動く）                                                                                       |
