...
| [~] | A-4    | **手動テストの通し実施** 149 件のうち **110 件は自動テストで担保**（`[a]` 印。根拠のテストを期待欄に書いた）。**残り 39 件が人の目で見るもの** | 内訳: 見え方（ダーク・余白・行の高さ）26、IME 3、外部エディタ 5、貼り付け 2、配布 3。配布は Developer ID 待ち                                                                                                                                                      | 実機                                                              |      |
...
| [ ] | C-9    | **`Cmd+P` を印刷に戻す** 今は PDF 書き出しに割り当てている。macOS では `Cmd+P` は印刷ダイアログが慣習で、そこから PDF も出せる                                                                                   | 慣習に合わせるほうが迷わない。要合意（今の割り当てを気に入っているなら変えない）                                                            | 小   |
...
| これから C（C-1〜C-10） | 0/10  | UI/UX の提案。C-1 のみ不具合（既知・未修正）         |
