...
1. **B-1 を先に。** 他と独立していて、既存メソッドの配線が中心。効き目がすぐ出る
2. **B-2 を次に。** ここを通さないと B-3〜B-5 はどれも書き出しで破綻する
3. B-3 → B-5(a) → B-4(a) の順。B-4(b) / B-5 のエディタ内表示は、配布サイズと
   起動時間を実測してから判断する
...
| これから B（B-1〜B-5） | 3/5   | B-1〜B-3 完了。残りは Mermaid（B-4）と数式（B-5）    |
