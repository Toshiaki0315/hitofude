...
| [ ] | A-2    | **画像・添付**                 | `Vault.attachments_dir` は作るだけで**呼び出し 0 回**。貼り付け → `attachments/` へ保存 → `![](相対パス)` を挿入まで。**本文中への表示は別物**（`QPlainTextEdit` は画像を持てないので行の高さを確保して `paintEvent` で描く。R5 と同じ土俵）。まずは「貼れて、HTML/PDF には出る」で切る | 貼り付け・書き出し・ファイル名の衝突・vault 外への参照 | 大 |
| [ ] | A-3    | **ノートの名前を変更**         | タイトルは本文の H1 から導かれる（`document.title_of`）ため、ファイル名だけ変えても一覧の表示は変わらない。**H1 を書き換える**か**ファイル名を独立させる**かの判断が要る。前者を推すが R1 に触れるので `docs/adr/` に記録を残す                                                         | 設計を決めてから                                       | 中 |
| [ ] | A-4    | **手動テストの通し実施**       | `docs/manual_test.md` が 68 項目未消化。IME・外部エディタ併用・データを失わないことなど、**自動テストでは担保できないもの**。実機でしか通せない                                                                                                                                         | —                                                      | —  |
| [ ] | A-5    | `ui/main_window.py` の分割     | 1,119 行。保存フロー・索引・検索・書き出し・レイアウト・ノート操作が 1 クラスに同居。**急がない**が、次に大きいものを足す前にノート操作あたりを切り出すと楽になる。回帰テストが揃っている今なら安全に動かせる                                                                           | 既存テストが緑のまま                                   | 中 |
...
| これから（A-1〜A-5）   | 0/5   | 未着手。着手前に合意を取る                           |
