...
|     | タスク | 内容                                                 | 理由                                                                                 | 規模 |
| --- | ------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------ | ---- |
| [ ] | E-1    | **チェックボックスをクリックで切り替え**             | 今は `Cmd+Shift+T` のみ。他のエディタはクリックで切れる                              | 小   |
| [ ] | E-2    | **ブラウザで確認**（書き出さずに既定ブラウザで開く） | **画面で見られない Mermaid・数式・コードの色がすぐ見える。** 既存の `to_html` を使う | 小   |
| [ ] | E-3    | **HTML をクリップボードへコピー**                    | Qiita・メール・Slack にファイル経由せず貼れる                                        | 小   |
