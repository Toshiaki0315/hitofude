...
| [ ] | B-6    | **コードの色分け** ` ```python ` の言語指定を見て予約語・文字列・コメントに色を付ける。**Pygments**（純 Python・4.9MB・598 言語・実測）。書き出しはインライン style の HTML を出すだけで、**ブラウザにも PDF にも色が乗る**（画素で確認済み）。`core/html.py` の描画規則に足す | 画面側は行をまたぐ状態が要る。**フェンス全体を 1 回解析してキャッシュし、各行はそこから引く**（行ごとに解析し直すと 100 行で 6.4ms×100 になり破綻）。§6.6 の 16ms に収まるかは着手時に実測する。ダーク用の配色も要る | 書き出しのみ=小〜中 / 画面も=中 |

### 進め方の推奨

1. ~~B-1~~ → ~~B-2~~ → ~~B-3~~ まで完了
2. ~~B-5~~ 完了（ADR-0009）。数式は JavaScript 無しで済んだ
3. **次は B-6 を推す。** B-4 と違って**画面側も現実的に手が届く**
   （Mermaid は実装が JavaScript しか無いのが制約）。まず書き出しだけ入れて、
   画面は性能を測ってから判断する
4. B-4 は最後。エディタ内表示（QtWebEngine）は配布サイズと起動時間を
   実測してから判断する
...
| これから B（B-1〜B-6） | 4/6   | B-1〜B-3・B-5 完了。残りは B-4（Mermaid）と B-6      |
