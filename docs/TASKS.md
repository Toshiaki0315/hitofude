...
| [~] | A-2    | **画像・添付** | **前半は完了。** 貼り付け・ドロップで `attachments/` へ保存し `![](相対パス)` を挿入。HTML は data URI で埋め込み、PDF にも出る。**残りは本文中への表示**（`QPlainTextEdit` は画像を持てないので行の高さを確保して `paintEvent` で描く。R5 と同じ土俵で調査から） | 貼り付け・ドロップ・書き出し・vault 外の拒否 | 残り: 大 |
