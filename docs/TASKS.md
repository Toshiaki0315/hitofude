...
| [ ] | B-5    | **数式** `$...$` と `$$...$$`。構図は B-4 と同じで、KaTeX / MathJax も JavaScript。書き出し HTML は MathJax（`tex-svg`、Web フォント不要）の同梱で足りる。**エディタ内と PDF には画像化が要る**。純 Python の候補は matplotlib の `mathtext`（LaTeX 本体は不要）だが依存が重く、要調査                                              | 画像にできれば ADR-0004 の仕組み（行高を文字サイズで作る＋キャッシュ）がそのまま使える。**既にある資産で乗る**             | 書き出しのみ=中 / エディタ内=大 |
...
1. ~~B-1~~ → ~~B-2~~ → ~~B-3~~ まで完了
2. **次は B-6 を推す。** B-4 / B-5 と違って**画面側も現実的に手が届く**
   （Mermaid も数式も実装が JavaScript しか無いのが制約）。B の中で費用対効果が
   いちばん高い。まず書き出しだけ入れて、画面は性能を測ってから判断する
3. そのあと B-5(a) → B-4(a)。B-4(b) / B-5 のエディタ内表示は、配布サイズと
   起動時間を実測してから判断する
...
| これから B（B-1〜B-6） | 3/6   | B-1〜B-3 完了。次は B-6（コードの色分け）を推す      |
