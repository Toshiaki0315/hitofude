...
| [ ] | B-3    | **Qiita 記法** `:::note info / warn / alert`、` ```lang:filename `、脚注 `[^1]`。エディタ側は `core/block_parser.py` に行の分類を足し、枠は `painter_overlay` で描く（R5 のためブロック書式は使わない）。書き出しは B-2 の上に `mdit-py-plugins`（`container` / `footnote`）                                                                                            | **どの記法を採るかを先に決める。** `:::` は CommonMark ではないので他のエディタでは素のテキストに見える（R1 は保たれる） | 中〜大 |
...
| これから B（B-1〜B-5） | 2/5   | B-1・B-2 完了。B-3〜B-5 の土台が乗った               |
