...
|     | タスク | 内容                                                                                                                                                                                                                                                                                                                                                                                                    | 前提・注意                                                                                                       | 規模 |
| --- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ---- |
| [ ] | B-1    | **書式ツールバー（右ペイン上部）** アイコンのボタンで太字・斜体・コード・打ち消し・マーカー・リンク・見出し・箇条書き・番号付き・引用・チェックボックス・表。**装飾系は既に実装済み**（`editor_widget.toggle_strong()` ほか）なので大半は配線。足りないのは行単位のトグル（箇条書き・番号付き・引用）で、`core/commands.py` に足す。アイコンは `ui/icons.py` に `Glyph` を追加（絵文字も画像ファイルも使わない既存方針）。表示 / 非表示を `layout/toolbar_visible` に覚える | ボタンに**フォーカスを渡さない**（`NoFocus`）。渡すと選択が外れてトグルが空振りする。IME 変換中は無効（R6）。1 押し = Undo 1 段 | 中   |
| [ ] | B-2    | **書き出しを markdown-it-py 直描画へ移す**（B-3〜B-5 の土台） `setMarkdown()` をやめ、既に依存している markdown-it-py で HTML を組む。PDF は生成した HTML を `QTextDocument.setHtml()` に渡す。R2 は「編集モデルに使わない」なので**この移行は R2 をより強く守る側**（例外が消える）                                                                                                    | 書き出しの出力が全面的に変わる。既存の書き出しテストは総取り替え。Qt のリッチテキストが受ける HTML/CSS の範囲を先に実測すること | 大   |
...
| これから B（B-1〜B-5） | 0/5   | 記法の拡張。**B-2 が B-3〜B-5 の前提**               |
