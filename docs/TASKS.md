...
| [ ] | B-4    | **Mermaid** ` ```mermaid ` を図にする。Mermaid は JavaScript 実装で、**Python の代替が無い**ことが制約。(a) `mermaid.min.js` を同梱して**書き出した HTML の中でだけ**図にする／(b) `QtWebEngine` を積んでエディタ内でも図にする／(c) Node の `mmdc` を外部プロセスで呼ぶ                                                                                        | (c) は Node 依存で配布が破綻するので却下。**(a) から始めるのを推す。**(a) では PDF に図が出ない（`QPrinter` 経由で JS が動かない）。(b) は `.app` の肥大と起動時間を先に実測 | a=中 / b=大 |
...
4. **残るは B-4（Mermaid）。** 実装が JavaScript しか無いのが制約で、
   まず同梱 JS で書き出しだけ。エディタ内表示（QtWebEngine）は配布サイズと
   起動時間を実測してから判断する
...
|     | タスク | 内容                                                                                                                                                                                                  | 根拠                                                                              | 規模 |
| --- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---- |
| [ ] | C-1    | **表のヘッダと本文の桁ずれ**（不具合） ヘッダを太字にしているぶん幅が広がり、罫線が本文と 7〜14px ずれる。「太字をやめる」「罫線を本文の行から引く」「字送りを変えずに太らせる」の 3 案      | 既知・報告済み・未修正。提案ではなく**バグ**                                      | 小〜中 |
| [ ] | C-2    | **アウトライン（見出しの目次）** 長いノートで迷子になる。見出しの一覧を出して飛べるようにする                                                                                                     | `core/block_parser.py` が既に見出しと深さを返すので安い。右ペインの袖か `Cmd+R` の一覧 | 中   |
| [ ] | C-3    | **一覧の並び順を選べる** 今は「ピン → 更新日時」の固定（`storage/index_db.py:253` ほかに直書き）。作成日順・名前順を選べるようにする                                                              | `ORDER BY` が 4 箇所に散っているので、まとめる作業を兼ねる                        | 小〜中 |
| [ ] | C-4    | **タグの入力補完** `#` を打った時点で既存のタグを出す。今は綴りを覚えていないと揺れる（`日報` と `日報メモ` が別タグになる）                                                                    | `tags` テーブルが既にある。`ui/quick_open.py` の絞り込みが流用できる              | 小   |
| [ ] | C-5    | **保存済みの合図** 今は未保存の `•` が消えるだけで、「書けた」ことが積極的に分からない。ステータスバーに最終保存時刻を出す                                                                       | データを預ける道具として、**書けたことが見えるのは安心に直結する**                | 小   |
| [ ] | C-6    | **空状態の案内** ノートが 0 件のとき一覧が真っ白になる。「＋ で新規作成」と出す                                                                                                                  | 初回起動の第一印象。`ui/note_list_pane.py` に `＋` は既にある                     | 小   |
| [ ] | C-7    | **ショートカット一覧の表示** 今はメニューを開かないと分からない。`Cmd+?` で一覧を出す                                                                                                            | `ui/menus.py` が既に一覧を持っているので、そこから生成できる（二重管理にしない）  | 小   |
| [ ] | C-8    | **直近のノートの履歴** 今の `Cmd+[` は 1 つ前に戻るだけ。数件の履歴を持って行き来する                                                                                                            | `session/last_note` を 1 件から数件に広げる                                       | 小   |
...
| これから B（B-1〜B-6） | 5/6   | 残るは B-4（Mermaid）のみ                            |
