...
| [x] | B-5    | **数式** `$...$` / `$$...$$`。**`latex2mathml` で MathML にする**（352KB・純 Python・変換 0.3ms）。JavaScript は同梱しない。今のブラウザがそのまま組む。画面はインライン数式を等幅にして式だと分かるようにする | ADR-0009。**PDF だけ LaTeX のまま出す**（Qt は MathML を解さず `$E=mc^2$` を `E=mc2`、`\frac{a}{b}` を `ab` にする）。matplotlib で絵にすれば PDF も画面も満たせるが **74MB**（実測）で釣り合わない。誤検出は 3 段で抑えた | 完了 |
