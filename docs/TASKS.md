...
| | タスク | 参照 |
|---|---|---|
| [ ] | 2-1 | `editor/highlighter.py`: ブロック状態のビットフラグ（コードフェンス / front matter / 表 / 引用深さ） | §6.3 |
| [ ] | 2-2 | 行頭マーカー判定 → `BlockInfo` 生成、`BlockData(QTextBlockUserData)` に格納 | §6.2 |
| [ ] | 2-3 | `inline_scanner` の結果を `setFormat()` で適用 | §6.3 |
| [ ] | 2-4 | マーカー隠蔽 `setFontPointSize(0.5)` | §3.3 / R4 |
...
| [ ] | 2-12 | **日本語 IME 実機確認**（R2 の早期検証） | §11 R2 |

**完了条件**（spec §9 より）
- [ ] 日本語で `**太字**` → 確定と同時に太字、`**` が消える
- [ ] カーソルを太字の中に入れると `**` が現れる
- [ ] コードブロック内では装飾が一切効かない
- [ ] `Cmd+Z` 1 回で直前の入力が戻る（Undo 汚染なし）
...
| | タスク | 参照 |
|---|---|---|
| [ ] | 3-1 | `editor/input_handler.py`: リスト継続 / 空項目の 2 段階解除 / 番号リスト | §5.5 |
| [ ] | 3-2 | 引用継続、Tab / Shift+Tab インデント（リスト行のみ） | §5.5 |
| [ ] | 3-3 | オートペア（選択範囲を囲む）、URL ペーストでリンク化 | §5.5 |
| [ ] | 3-4 | **IME ガード**: 変換中は Enter/Tab の特殊処理を全無効化 | §5.5 / R6 |
| [ ] | 3-5 | `editor/commands.py`: `Cmd+B/I/E/K/Shift+X/Shift+H` のトグル（外す処理も必須） | §5.4 |
| [ ] | 3-6 | 見出しレベル ±1、チェックボックス切替 | §5.4 |
...
| [ ] | 5-8 | `scripts/gen_dummy_vault.py`: 5,000 件のダミーノート生成 | 性能検証用 |

**完了条件**: 5,000 件でリストのスクロールが滑らか、検索 200ms 以内（実測値を記録する）。
...
| | タスク | 参照 |
|---|---|---|
| [ ] | 6-1 | ソースモード `Cmd+/`、フォーカスモード、タイプライタモード | §5.4 |
| [ ] | 6-2 | `Cmd+Shift+C`（プレーンテキストコピー） | §5.4 |
| [ ] | 6-3 | HTML / PDF エクスポート（**ここでのみ `setMarkdown()` 可**） | §9 / R2 |
| [ ] | 6-4 | アプリアイコン `.icns`、About ダイアログ | §8.1 |
| [ ] | 6-5 | `setup.py`（py2app）→ ビルド → 署名 → 公証 → DMG | §8.1–8.3 |
| [ ] | 6-6 | クラッシュリカバリ（未保存バッファの退避） | §9 |
| [ ] | 6-7 | 巨大ファイルガード（2MB / 20,000 行超は装飾無効化 + 警告） | §6.6 / R7 |
| [ ] | 6-8 | 手動テストチェックリスト（IME / 配布）を `docs/manual_test.md` に | §10 |
