...
- WYSIWYG な表エディタ（v1 は表を「等幅フォント + 罫線揃え」で扱う）
...
| エディタウィジェット | `QPlainTextEdit` サブクラス | — | `QTextEdit` より軽い。ただし**画像インライン表示を v1.1 で入れるなら `QTextEdit`** を選ぶ（§4.1） |
...
| **画像のインライン表示** | **×** | **◎**（`QTextDocument.addResource` + `QTextImageFormat`） |
| 表の罫線描画 | × | △（`QTextTable`。ただしソース保持と両立が難しい） |

**v1 の判断: `QPlainTextEdit` を採用**。

画像は「`![alt](path)` の行の下に別ウィジェットで縮小表示」ではなく、**v1 ではリンクとして扱い、`Cmd+クリック` でプレビューをポップオーバー表示**する。インライン画像は v1.1 で `QTextEdit` への移行と合わせて再検討する。
...
| 等幅フォント | 既定 `SF Mono` 13pt（コード、コードブロック、表） |
...
- ファイル名は `sanitize(タイトル) + .md`。重複時は `-2`, `-3` を付与。タイトル変更時はファイルをリネームする（旧名は `.trash` に残さない）。
