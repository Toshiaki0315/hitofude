...
**本アプリの方針**: 先行アプリの *UX*（マーカーが隠れる、タグがインラインで書ける、サイドバーがタグツリー）は取り入れるが、*記法* は **CommonMark + GFM 拡張**を基準とし、**保存形式も素の Markdown とする**（G3）。
> **例外 1 つ**: 4 字下げのコードブロックは**設定で切れる**（[ADR-0033](adr/0033-indented-code-option.md)）。既定は on＝準拠のまま。貼り付けで意図せずコードに化けるのが煩わしい人のための逃げ道で、保存形式は変わらない。GFM に無い便利記法のうち、以下だけを独自拡張として採用する。
...
| コードブロック | 等幅 + ブロック背景（~~左 4px アクセントバー~~ → **ADR-0008 で削除**。ファイル名を出す） |
| 引用 `>` | 左に 3px の縦バー、左マージン 16px、文字色 60% グレー |
| リンク | アクセントカラー + 下線。`Cmd+クリック` で既定ブラウザを開く |
| タグ `#tag` | 背景色付きピル形状。~~クリック~~ → **`Cmd+クリック`**（ADR-0010）でそのタグで絞り込み |
| ハイライト `::` | 背景 `#FFF3A0` |
...
QTextDocument.contentsChange(position, charsRemoved, charsAdded)
    ...
    │
    ├──▶ [廃止] デバウンス 400ms のダーティ範囲再解析（ADR-0014 で不要と判断）
    │         行またぎの構造は classify_line + ブロック状態のビットフラグだけで
    │         追跡できた。parse() は正しさを検証する参照実装として残す
    │
...
└── .OboeGaki/                          ← アプリの管理領域（ユーザーは触らない）
    ├── index-hitofude.sqlite       ← 検索インデックス（キャッシュ。消えても再構築可能。ADR-0034）
    ├── index-hitofude.sqlite-wal
...
- **索引は完全なキャッシュ**。削除しても `.md` から全再構築できること。真実は常にファイル側にある。これが G3 の担保。名前は `storage/index_db.INDEX_FILE` が唯一の出所で、素の `index.sqlite` は作り直し版（Tauri/Rust）が使うので避ける（[ADR-0034](adr/0034-index-file-per-implementation.md)）。
...
| タイトル | **最初の H1 → 無ければ最初の非空行 → ~~無ければファイル名~~** の順で決定 → **ADR-0015 で変更**: 最後は「無題」。ファイル名はタイトルの写し（ADR-0005）なので、出どころに使うと本文を消してもタイトルが残る |
...
→ **ADR-0013 で変更**: どのモードでも**完全に隠す**（Raw でも出さない）。`id` / `created` はアプリの管理情報で、書く人が触るものではない。
...
- **完了条件**:
  ...
  - `.OboeGaki` の索引ファイルを消して再起動 → 完全に復元される
...
| R9 | `QSyntaxHighlighter` は行またぎの構造（表・入れ子リスト）を単独で判断できない | 表の装飾がずれる | ブロック状態のビットフラグ ~~+ デバウンスした `block_parser` の結果を併用~~（§6.3 / ADR-0014: ビットフラグのみで足りた） |
