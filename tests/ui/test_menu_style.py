...
class TestTooltip:
    """ネイティブのツールチップも黒地に白（ユーザー要望 2026-08-24）。

    主役は自前の窓（`ui/tooltip.py`。角丸まで出る。試験は
    tests/ui/test_tooltip.py）。ここで見るのは**採用していない場所に
    出るネイティブの保険**——色はパレットで、余白はスタイルの寸法値で
    整える。スタイルシートは使わない（置ける場所がどこも壊れる。
    `app.apply_tooltip_colors` に実測を書いた）。
    """
