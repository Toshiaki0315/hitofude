...
class TestSortMenu:
    ...
    def test_三角を出さない(self, pane) -> None:
        ...
        場所を広げて凌いでいたが、**そもそも三角を出さない**形にした
        （文字＋三角をやめて、正方形のアイコンボタンにした）。重なる
        余地が無くなったので、ここは「出していないこと」を見る。
        """
        assert "menu-indicator" in pane.sort_button.styleSheet()

    def test_正方形に収まっている(self, pane) -> None:
        assert pane.sort_button.width() == pane.sort_button.height()
