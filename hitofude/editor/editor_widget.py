...
CONTENT_MARGIN = 40.0
"""**12 → 24 → 40px と広げた**（ユーザー要望 2026-08-26）。

24 でも、見出しの開閉三角（8px）が縁の x=2 に貼り付いて窮屈だった。
三角を余白の**真ん中**に置き、縁→三角と三角→本文を 16px ずつにする
（`painter_overlay._fold_marker`）。

広げると本文の使える幅がそのぶん狭くなる。表は収まらなければ**折り返す**
作りになった（ADR-0028 / 0029）ので、桁が足りなくなって崩れることはない。
"""
